...
persisted = {'stone': c, 'sword': ec, 'bow': ec, 'wand': ec, 'glove': ec,
             ...
             'grid': c + ('tiles','x','y'),
             'player': ('name', 'squads'),
             'initial_state': ('start_time', 'init_locs', 'units', 'grid',
                               'owners', 'player_names',)
             }
        ...
        for key in persisted[kind]:
            ...
            if key == 'tiles':
                ...
                for (k, v) in obj.__dict__[key].items():
                    new_x = {}
                    for (l, w) in v.items():
                        new_x.update({l: get_persisted(w, debug)})
                    new_dict['tiles'].update({k: new_x})
            #For Logs
            elif key == 'units':
                new_dict['units'] = {}
                for (n, s) in obj.__dict__[key].items():
                    if debug == True: print "n: %s, s: %s" %(n,s)
                    new_dict['units'][n] = get_persisted(s, debug)
            
            elif key == 'init_locs':
                new_dict['init_locs'] = {}
                for (n, s) in obj.__dict__[key].items():
                    new_dict['init_locs'][n] = get_persisted(s, debug)
                ...
                for (n, s) in obj.__dict__[key].items():
                    if debug == True: print "n: %s, s: %s" %(n,s)
                    new_dict['body'][n] = get_persisted(s, debug)
            
            elif key == 'squad_list':
                sl = []
                for s in obj.__dict__[key]:
                        sl.append(get_persisted(s, debug))
                new_dict['squad_list'] = sl
            
            elif key == 'players':
                ps = []
                for p in obj.__dict__[key]:
                        ps.append(get_persisted(p, debug))
                new_dict['players'] = ps
            #??
            elif key == 'owners':
                new_dict['owners'] = {}
                for key, value in obj.__dict__[key].items():
                    new_dict['owners'][str(key)] = value
            ...
            #special case for the .data of Squad:
                ...
                for item in obj.__dict__[key]:
                    data.append(get_persisted(item, debug))
                new_dict[str(key)] = data
            
            #if value are persisted, call self
            elif other_kind in persisted.keys():
                new_dict[str(key)] = get_persisted(obj.__dict__[key], debug)
            #else put value into new_dict
            else:
                new_dict[str(key)] = obj.__dict__[key]
        return {str(kind): new_dict} 
    else:
        if kind == 'persistentmapping':
            return dict(obj) #MAGIC INCANTATION.
        else:
            return obj
